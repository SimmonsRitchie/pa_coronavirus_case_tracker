...
export const joinData = ({dataset1, dataset2, leftJoinOn, rightJoinOn, appendKey}) => {
  ...
  */
  ...
  const mergedData = [];
  dataset1.forEach((data1Item, i) => {
    let newItem;
    dataset2.forEach(data2Item => {
      if (data1Item[leftJoinOn].toLowerCase() === data2Item[rightJoinOn].toLowerCase()) {
        newItem = { ...data1Item, [appendKey]: data2Item[appendKey] };
      }
    });
    newItem = newItem ? newItem : {...data1Item, [appendKey]: null}
